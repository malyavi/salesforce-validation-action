 ...
 *
 * Vendor-prefixed on purpose: the tag is an HTML comment in a pull request body
 * that any number of tools write into, and an unprefixed `pr-status-comment`
 * is the marker somebody else's bot would plausibly choose too — which would
 * have the two of them overwriting each other's body.
 */
export const DEFAULT_COMMENT_TAG = '<!-- malyavi-pr-status-comment -->';
